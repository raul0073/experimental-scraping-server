...
class SoccerDataService:
    def __init__(self, league: str, season: str):
        ...
        logging.info(f"SoccerDataService initialized with: {self.league} / {self.season}")

    def get_team_stats(self, team: str, stats_type: StatsOptions, against: bool) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            teams_data = self.fbref.read_team_season_stats(stat_type=stats_type, opponent_stats=against)
            ...
            team_data = teams_data[teams_data["team"].str.lower() == team.lower().strip()]
            if stats_type in LABELS_CONFIG:
                team_data = SoccerDataUtils.rename_columns(team_data, LABELS_CONFIG[stats_type])
    ...
    def get_player_stats_df(self, team: str, stats_type: str, player_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if team_df.empty:
                raise ValueError(f"No stats found for player '{player_name}' in team '{team}'.")

            # Rename columns if the stats_type is valid
            if stats_type in LABELS_CONFIG:
                team_df = SoccerDataUtils.rename_columns(team_df, LABELS_CONFIG[stats_type])
            else:
            ...
            player_df = team_df[team_df["player"].str.lower().str.contains(player_name.lower().strip(), na=False)]
            if player_df.empty:
                raise ValueError(f"No stats found for player '{player_name}' in team '{team}'.")
        ...
        try:
            ...
            for stat_type in StatsOptions:
                ...
                if stat_type.value in LABELS_CONFIG:
                    team_df = SoccerDataUtils.rename_columns(team_df, LABELS_CONFIG[stat_type.value])
